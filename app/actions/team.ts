...
export async function addTeamMember(appId: string, formData: FormData) {
  try {
    ...
    if (!targetUser) {
      return { error: "User not found. They must register for a Lycen account first." }
    }
...
export async function removeTeamMember(appId: string, memberRelationshipId: string) {
  const user = await requireAuth()
  ...
  const target = await queryOne<{ user_id: string }>("SELECT user_id FROM application_members WHERE id = ?", [memberRelationshipId])
  
  if (target?.user_id === user.id) {
    return { error: "You cannot remove yourself. Delete the application instead." }
  }

  await query("DELETE FROM application_members WHERE id = ?", [memberRelationshipId])
